...
* **`order`**
  Playback order of patterns (1-indexed).
...
* **`tempo`**
  ...
  Setting the tempo to `0` restores file-defined tempo changes.

* **`jumps`**
  A list of jump points of the form `[from]>[to]`.
  When playback reaches the end of bar `from`, it jumps to the start of
  bar `to`.
  A `to` value of `0` stops playback.

* **`barlength`**
  Number of ticks per full measure. Defaults to `1`.
...
* **`tempo`**
  Overrides file tempo behavior as described above
