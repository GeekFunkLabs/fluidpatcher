...
class CCLink:
    ...
    def __init__(self, fluid, target, link, type, xfrm=RouterSpec(0, 127, 1, 0), **kwargs):
        self.fluid = fluid
        self.target = target
...
class Patcher:
    ...
    # select :patch by index, name, or passing dict object
        ...
        for link in self.cc_links:
            if link.type in 'effect fluidsetting':
    ...
    # load a single :soundfont and scan all its presets
        ...
        for bank in range(MAX_BANK):
            for prog in range(MAX_PRESET):
                ...
                if not name:
                    continue
                self.sfpresets.append(SFPreset(name, bank, prog))
    ...
    def link_cc(self, target, chan=0, cc=0, type='effect', **kwargs):
        if chan > 0:
            self.cc_links.append(CCLink(self.fluid, target, '%s/%s' % (chan, cc), type, **kwargs))
        else:
            kwargs['type'] = type
            self.cc_links.append(CCLink(self.fluid, target, **kwargs))
